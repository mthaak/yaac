...
__copyright__ = 'Copyright (C) 2000-2005 Erik Max Francis, 2004-2006 David Keeney'
__license__ = 'LGPL'
...
class Interface(object):
    ...
    def _joystickMotion(self, axis, val):
        """Joystick motion wrapper.  Private method. Override
        joystickMove rather than this.
        """
        if axis > 1: return
        self.joystickPosition[axis] = val
        self.joystickMove(self.joystickPosition)
    ...
    def joystickMove(self, pos):
        """The joystick moved to pos (0-1, 0-1)."""
        raise NotImplementedError('joystickMove')

    def joystickButtonDown(self, button):
        """The specified joystick button was clicked.
        @param button: which button was pressed
        """
        raise NotImplementedError('joystickDown')

    def joystickButtonUp(self, button):
        """The specified joystick button was released.
        @param button: which button was released
        """
        raise NotImplementedError('joystickUp')

    def joystickHatMove(self, pos):
        """The joystick hat moved to pos (0-1, 0-1)."""
        raise NotImplementedError('joystickHatMove')
...
class BareBonesInterface(Interface):
    ...
    def event(self, ev):
        ...
        if ev.type == VIDEORESIZE:
            ...
            self._joystickMotion(ev.axis, ev.value)
        elif ev.type == JOYBUTTONUP:
            self.joystickButtonUp(ev.button)
        elif ev.type == JOYBUTTONDOWN:
            self.joystickButtonDown(ev.button)
        elif ev.type == JOYHATMOTION:
            self.joystickHatMove(ev.value)
        elif ev.type == QUIT:
            self.quit()
    ...
    def joystickMove(self, pos):
        """The joystick moved to pos (-1-1, -1-1)."""
        pass
    ...
    def joystickHatMove(self, pos):
        """The joystick hat moved to pos (-1-1, -1-1)."""
        pass
...
class EngineSuperShutdownException(Exception):
    """Exception that exits engine, and gets reraised. Can be used
    to signal program stop, in addition to engine stop.
    """
    pass
...
class Engine(object):
    ...
    def dumpStats(self, empty=None):
        ...
        '  ( %.4f %.4f %.4f ) ' % cam.center
        print
        '  camera viewbox bounds  (l/r) '
        print
        '  ( %.4f %.4f ) ' % (cam.left, cam.right)
        print
        '  camera viewbox bounds  (t/b) '
        print
        '  ( %.4f %.4f ) ' % (cam.top, cam.bottom)
        print
        '  camera viewbox bounds  (n/f) '
        print
        '  ( %.4f %.4f ) ' % (cam.near, cam.far)
